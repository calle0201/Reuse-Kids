...
if( !empty($_POST['admin']) && !empty($_POST['adminPassword'])) 
{
	...
	$email =  mysqli_real_escape_string($connect, $_POST['admin']);
	
	$loginPassword =  mysqli_real_escape_string($connect, $_POST['adminPassword']);
	...
	if($num >0)  //användaren hittades
	{
		...
		$ver_password = password_verify($adminPassword, $db_password);
