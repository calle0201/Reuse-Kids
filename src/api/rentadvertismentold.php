...
if(!empty($title) && !empty($text) && !empty($price) && !empty($picture) && !empty($name) && !empty($email) && !empty($municipality) && !empty($county) && !empty($date)) 
{
        ...
        $sql = "INSERT INTO rentadvertisment ( title, text, picture, size, price, userid, phone, name, email, date, enddate, municipality, county, marketingpicture) VALUES ('$title','$text','$picture','$size','$price',(select userid FROM user WHERE email='$email'),'$phone','$name', '$email', '$date','$enddate','$municipality','$county','$marketingpicture')";
