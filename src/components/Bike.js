...
class Bike extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/bike.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
            <Helmet>
                <title>Begagnad cykelutrustning </title>
                <meta name="description" content="Webbsida för begagnade tävlingscyklar, cykelskor, cykelhjälm, cykel, mountainbike, racercykel." />
            </Helmet>
            <h1 className="Advertisment_title">Cykelsport</h1>
            <h3 className="Advertisment_subtitle">Här finns det cyklar och cykelutrustning som skor, kläder och cykeldator. Är sidan tom finns det inga annonser för tillfället.</h3>
