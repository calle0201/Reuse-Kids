...
class Wrestling extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/wrestling.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad brottningsutrustning </title>
                <meta name="description" content="Webbsida för begagnade brottningsskor, brottningstrikåer, brottarskydd och brotttardocka." />
            </Helmet>
        <h1 className="Advertisment_title">Brottning</h1>
        <h3 className="Advertisment_subtitle">Här finns det brottningsutrustning som skor och kläder. Är sidan tom finns det inga annonser för tillfället.</h3>
