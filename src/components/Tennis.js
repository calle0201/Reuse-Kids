...
class Tennis extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/tennis.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad tennisutrustning </title>
                <meta name="description" content="Webbsida för begagnade tennisrack, tennisnät, tennisväskor och tennisskor." />
            </Helmet>
        <h1 className="Advertisment_title">Tennis</h1>
        <h3 className="Advertisment_subtitle">Här finns det tennisutrustning som racket, skor och kläder. Är sidan tom finns det inga annonser för tillfället.</h3>
