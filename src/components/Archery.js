...
class Archery extends Component {
    ...
    componentDidMount() {
      ...
      this.props.hideAds(Ads);
        ...
        fetch('https://reusesport.se/APIs/archery.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
          <Helmet>
                <title>Begagnade pilbågar och bågskytteutrustning </title>
                <meta name="description" content="Webbsida för begagnade pilbågar och utrustning för bågskytte, compoundbåge, recurvebåge, koger, pilar, stabilisator." />
            </Helmet>
            <h1 className="Advertisment_title">Bågskytte</h1> 
            <h3 className="Advertisment_subtitle">Här finns det pilbågar av olika sorter och utrustning till dem. Är sidan tom finns det inga annonser för tillfället.</h3>
