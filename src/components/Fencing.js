...
class Fencing extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/fencing.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad fäktningsutrustning </title>
                <meta name="description" content="Webbsida för begagnade värjor, bröstskydd, masker, plastrong, fäktskor ocxh handske." />
            </Helmet>

        <h1 className="Advertisment_title">Fäktning</h1>
        <h3 className="Advertisment_subtitle">Här finns det fäktningsutrustning som värjor, kläder och skydd. Är sidan tom finns det inga annonser för tillfället.</h3>
