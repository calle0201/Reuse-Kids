...
class Ski extends Component {
  ...
  componentDidMount() {
    ...
    fetch('https://reusesport.se/APIs/ski.php', obj)
    .then(res => {
        if (res.ok) {
          return res.json();
        } else {
      ...
      .then(json => {
        ...
        render() {
            return (
                <div>
                     <Helmet>
                        <title>Begagnad skidutrustning </title>
                        <meta name="description" content="Webbsida för begagnade skidor, längdskidor, slalomskidor, snowboard, stavar, skidfodral, bindningar, skidhållare, pjäxor och slalompjäxor." />
                    </Helmet>
                <h1 className="Advertisment_title">Skidåkning</h1>
                <h3 className="Advertisment_subtitle">Här finns det skidutrustning som slalomskidor, längskidor, kläder och pjäxor. Är sidan tom finns det inga annonser för tillfället.</h3>
