...
class Floorball extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/floorball.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad innebandyutrustning </title>
                <meta name="description" content="Webbsida för begagnade innebandyklubbor, innebandyglasögon, blad, innebandyskor, målvaktskläder och innebandyväskor." />
            </Helmet>
        <h1 className="Advertisment_title">Innebandy</h1>
        <h3 className="Advertisment_subtitle">Här finns det innebandyutrustning som innebandyklubbor, skor och skydd. Är sidan tom finns det inga annonser för tillfället.</h3>
