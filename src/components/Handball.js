...
class Handball extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/handball.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad handbollsutrustning </title>
                <meta name="description" content="Webbsida för begagnade handbollskläder, handbollsskor och handbollsskydd." />
            </Helmet>
            <h1 className="Advertisment_title">Handboll</h1>
            <h3 className="Advertisment_subtitle">Här finns det handbollsutrustning som skor, kläder och skydd. Är sidan tom finns det inga annonser för tillfället.</h3>
