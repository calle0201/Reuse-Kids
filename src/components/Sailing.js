...
class Sailing extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/sailing.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad seglingsutrustning </title>
                <meta name="description" content="Webbsida för begagnade kappseglingsbåtar, segelbåtar, segel, riggdetaljer, block, flytvästar, livflotte, flytplagg och räddningsväst." />
            </Helmet>
            <h1 className="Advertisment_title">Segling</h1>
            <h3 className="Advertisment_subtitle">Här finns det seglingsutrustning som tävlingsbåtar, segel, reservdelar och segelkläder. Är sidan tom finns det inga annonser för tillfället.</h3>
