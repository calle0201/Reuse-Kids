...
class Golf extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/golf.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnad golfutrustning </title>
                <meta name="description" content="Webbsida för begagnade golfklubbor, golfbagar, golfhandskar, golfvagnar, golfskor och golfkläder." />
            </Helmet>
        <h1 className="Advertisment_title">Golf</h1>
        <h3 className="Advertisment_subtitle">Här finns det golfutrustning som golfklubbor, bagar och skor. Är sidan tom finns det inga annonser för tillfället.</h3>
