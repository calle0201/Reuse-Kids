...
class Canoe extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/canoe.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnade kanoter </title>
                <meta name="description" content="Webbsida för begagnade kanoter, kanadensare, kajak, paddlar, kajakvagn, paddelpåse, flytväst, paddelväst, torrdräkt" />
            </Helmet>
            <h1 className="Advertisment_title">Kanot</h1>
            <h3 className="Advertisment_subtitle">Här finns det kanoter och utrustning som paddlar, flytvästar och kapell. Är sidan tom finns det inga annonser för tillfället.</h3>
