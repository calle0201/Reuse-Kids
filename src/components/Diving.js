...
class Diving extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/diving.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
             <Helmet>
                <title>Begagnade dykutrustning </title>
                <meta name="description" content="Webbsida för begagnade dykardräkter, dykarklockor, torrdräkt, våtdräkt, fenor, flaskor, snorkel och regulator." />
            </Helmet>
 
            <h1 className="Advertisment_title">Dykning</h1>
            <h3 className="Advertisment_subtitle">Här finns det dykutrustning som dykardräkter, tuber, masker och simfötter. Är sidan tom finns det inga annonser för tillfället.</h3>
