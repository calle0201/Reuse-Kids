...
class Athletics extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://reusesport.se/APIs/athletics.php', obj)
        .then(res => {
            if (res.ok) {
              return res.json();
            } else {
    ...
    render() {
      return (
        <div>
            <Helmet>
                <title>Begagnad friidrottsutrustning </title>
                <meta name="description" content="Webbsida för begagnad friidrottsutrustning, diskus, kula, spjut, höjdhopp, häcklöpning." />
            </Helmet>
            <h1 className="Advertisment_title">Friidrott</h1>
            <h3 className="Advertisment_subtitle">Här finns det friidrottsutrustning som diskus, spjut, träningshäckar och tidtagarur. Är sidan tom finns det inga annonser för tillfället.</h3>
